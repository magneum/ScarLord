import Database.notes_sql as sql
...
from Bytes.alternate import typing_action
import Database.rules_sql as rulessql
import Database.blacklist_sql as blacklistsql
from Database import disable_sql as disabledsql
import Database.locks_sql as locksql
...
def export_data(update: Update, context: CallbackContext):
    ...
    with open("SƈαɾLσɾԃ{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("SƈαɾLσɾԃ{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `SƈαɾLσɾԃ-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("SƈαɾLσɾԃ{}.backup".format(chat_id))  # Cleaning file
