...
from Bytes.handlers import CMD_STARTERS, SpamChecker
from Bytes.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "SƈαɾLσɾԃ.Skars." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "SƈαɾLσɾԃ.Skars." + args[0].rsplit(".", 1)[0]
